...
import bots.binance.util.TriangleLoader;
...
public class OptimizedMultiTriArbitrageBot {
    ...
    private static final Logger logger = LoggerFactory.getLogger(OptimizedMultiTriArbitrageBot.class);
    private static final List<TriangleLoader.Triangle> TRIANGLES = TriangleLoader.loadTriangles("triangular_pairs.json");


    private static final Set<String> uniquePairs = TRIANGLES.stream()
    	    .flatMap(t -> Arrays.stream(t.symbols))
            ...
            if (frame instanceof TextWebSocketFrame textFrame) {
                ...
                if (json.has(DATA)) {
                    ...
                    if (ExchangeRulesManager.isValidTrade(symbol, ask, bid)) {
                        ...
                        for (TriangleLoader.Triangle triangle : TRIANGLES) {
                            String p1 = triangle.symbols[0];
                            String p2 = triangle.symbols[1];
                            String p3 = triangle.symbols[2];

                            if (symbol.equals(p1) || symbol.equals(p2) || symbol.equals(p3)) {
